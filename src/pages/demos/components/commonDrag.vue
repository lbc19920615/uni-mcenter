  ...
  methods: {
    onChange(e, item) {
      ...
      if (e.detail.source === 'touch') {
        ...
        let canNotMoveImageListID = this.imageList.filter(v => v.holder).map(v => v.id)
        let moveoverItem = this.imageList[index]
        // console.log(canNotMoveImageListID, moveoverItem.id)
        if (moveoverItem && canNotMoveImageListID.indexOf(moveoverItem.id) < 0) {
