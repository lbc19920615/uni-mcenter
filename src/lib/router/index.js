...
export default class Router {
	...
	_changeRouter(type, params) {
		...
		const nextFunc = () => {
			let realParams = getRealParams(to);

			if (!realParams) {
				throw new Error('realParams null');
			}

			uni[type]({
				url: realParams.url,
				events: realParams.events,
				success: (arg) => {
					that.afterEachCallback(to, from)
					if (realParams.success) {
						realParams.success(arg);
					}
				},
				fail: err => {
				...
				complete: (arg) => {
					...
					if (realParams.complete) {
						realParams.complete(arg);
					}
				}
			})
		}
